                        ...
                        double (*crd)[3], uint64_t (*idx)[2] )
{
   ...
   int64_t bound[ MaxPth ][2], cpt, (*tab)[2];
   size_t NmbByt;
   double len = pow(2,64);
   ParSct *par = (ParSct *)ParIdx;
   ArgSct arg;
...
int ResizeType(int64_t ParIdx, int TypIdx, LplInt NmbLin)
{
   ...
   if(!par || (TypIdx < 1) || (TypIdx > MaxTyp) \
   || !(typ = &par->TypTab[ TypIdx ]) || (NmbLin < typ->NmbLin ) )
   {
      return(0);
   }
...
float LaunchParallel(int64_t ParIdx, int typ1, int typ2, \
                     void *prc, void *PtrArg )
{
   ParSct *par = (ParSct *)ParIdx;
   ...
   if( (typ1 < 1) || (typ1 > MaxTyp) || (typ2 < 0) \
   ||  (typ2 > MaxTyp) || (typ1 == typ2) )
   {
      return(-1.);
   }
   ...
   {
      void (*UsrPrc)(LplInt, LplInt, LplInt, void *) = \
         (void (*)(LplInt, LplInt, LplInt, void *))prc;
...
LplInt HilbertRenumbering( int64_t ParIdx, LplInt NmbLin, double box[6], \
                           double (*crd)[3], uint64_t (*idx)[2] )
{
   for(LplInt i=1;i<=NmbLin;i++)
      idx[i][0] = idx[i][1] = (uint64_t)i;
   return(1);
}

LplInt HilbertRenumbering2D(  int64_t ParIdx, LplInt NmbLin, double box[4], \
                              double (*crd)[2], uint64_t (*idx)[2])
{
   for(LplInt i=1;i<=NmbLin;i++)
      idx[i][0] = idx[i][1] = (uint64_t)i;
...
int LaunchPipeline(  int64_t ParIdx, void *prc, void *PtrArg, \
                     int NmbDep, int *DepTab )
{
   ParSct *par = (ParSct *)ParIdx;
