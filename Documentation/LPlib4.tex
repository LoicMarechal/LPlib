...
\Large March 2026 \\
\normalsize Document v1.82, library v4.03
\end{flushright}
...
\subsection{Colored grains scheduling}

THis kind of scheduling is used to handle concurent indirect memory writes, like the depdency loops seen before, but it uses a completely different kind of partitioning and scheduling. The whole mesh needs to be partitionned through the colgrain command.
...
Parameter  & type    & description \\
\hline
LibIndex   & int64\_t & instance number of \emph{LPlib} \\
\hline
...
Parallel memclear is only useful for sizes greater than 100 MB. Below this threashold, the system memset function is called sequentially. As this operation is purely memory bound, do not expect a scaling greater than 2 to 4.


\subsection{ParallelMemCopy}

\subsubsection*{Syntax}
\tt{code = ParallelMemCopy(LibIndex, dst table, src table, size);}
\normalfont
...
LibIndex   & int    & instance number of \emph{LPlib} \\
\hline
dst table  & void * & pointer to a memory area that will receive the copy \\
\hline
src table  & void * & pointer to a memory area that will be copied \\
\hline
size       & long   & number of bytes to be copied \\
\hline
\end{tabular}
...
\subsubsection*{Description}
It works similarly to the C library memcpy command.

Parallel memcopy is only useful for sizes greater than 100 MB. Below this threashold, the system memcpy function is called sequentially. As this operation is purely memory bound, do not expect a scaling greater than 2 to 4.
