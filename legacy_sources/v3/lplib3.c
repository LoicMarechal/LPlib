...
typedef struct
{
	...
	int NmbSmlWrk, NmbBigWrk, SmlWrkSiz, BigWrkSiz, DepWrkSiz, NmbDepWrd, *DepWrdMat, *RunDepTab;
...
typedef struct ParSct
{
	...
	int req, cmd, ClrLinSiz, *PipWrd, SizMul, NmbF77Arg;
...
long long InitParallel(LplInt NmbCpu)
{
	int i;
	long long ParIdx;
	ParSct *par;
...
void StopParallel(long long ParIdx)
{
	int i;
	PthSct *pth;
...
float LaunchParallel(long long ParIdx, LplInt TypIdx1, LplInt TypIdx2, void *prc, void *PtrArg)
{
	int i;
	PthSct *pth;
	ParSct *par = (ParSct *)ParIdx;
...
LplInt NewType(long long ParIdx, LplInt NmbLin)
{
	int i, TypIdx=0, idx;
	TypSct *typ;
	ParSct *par = (ParSct *)ParIdx;
...
LplInt ResizeType(long long ParIdx, LplInt TypIdx, LplInt NmbLin)
{
	int i, idx;
	TypSct *typ;
	ParSct *par = (ParSct *)ParIdx;
...
void FreeType(long long ParIdx, LplInt TypIdx)
{
	TypSct *typ;
	ParSct *par = (ParSct *)ParIdx;
...
LplInt BeginDependency(long long ParIdx, LplInt TypIdx1, LplInt TypIdx2)
{
	int i;
	TypSct *typ1, *typ2;
	ParSct *par = (ParSct *)ParIdx;
	...
	if( (typ2->NmbLin >= DefNmbDepBlk * par->NmbCpu) && (typ2->NmbLin >= typ1->DepWrkSiz * 32) )
	{
	...
	if(!(typ1->DepWrdMat = calloc(typ1->NmbSmlWrk * typ1->NmbDepWrd * par->SizMul, sizeof(int))))
		return(0);
	...
	for(i=0;i<typ1->NmbSmlWrk;i++)
	{
		...
		typ1->SmlWrkTab[i].DepWrdTab = &typ1->DepWrdMat[ i * typ1->NmbDepWrd * par->SizMul ];
	}
...
LplInt AddDependency(long long ParIdx, LplInt idx1, LplInt idx2)
{
	WrkSct *wrk;
	ParSct *par = (ParSct *)ParIdx;
	...
	if( !par || !par->CurTyp || !par->CurTyp->SmlWrkSiz || !par->CurTyp->DepWrkSiz || !par->DepTyp \
	|| (idx1 < 1) || (idx1 > par->CurTyp->NmbLin) || (idx2 < 1) || (idx2 > par->DepTyp->NmbLin) )
	{
		return(0);
	}
...
void AddDependencyFast(long long ParIdx, LplInt NmbTyp1, LplInt *TabIdx1, LplInt NmbTyp2, LplInt *TabIdx2)
{
	int i, j;
	ParSct *par = (ParSct *)ParIdx;
	WrkSct *wrk;
...
LplInt UpdateDependency(long long ParIdx, LplInt TypIdx1, LplInt TypIdx2, LplInt idx1, LplInt idx2)
{
	WrkSct *wrk;
	ParSct *par = (ParSct *)ParIdx;
	TypSct *typ1, *typ2;
	...
	if( (TypIdx1 < 1) || (TypIdx1 > MaxTyp) || (TypIdx2 < 1) || (TypIdx2 > MaxTyp) || (typ1 == typ2) \
	|| !typ1->NmbLin || !typ2->NmbLin || !typ1->SmlWrkSiz || !typ1->DepWrkSiz \
	|| (idx1 < 1) || (idx1 > typ1->MaxNmbLin) || (idx2 < 1) || (idx2 > typ2->MaxNmbLin) )
	{
		return(0);
	}
...
void UpdateDependencyFast(	long long ParIdx, LplInt TypIdx1, LplInt NmbTyp1, \
							LplInt *TabIdx1, LplInt TypIdx2, LplInt NmbTyp2, LplInt *TabIdx2)
{
	int i, j;
	ParSct *par = (ParSct *)ParIdx;
...
LplInt EndDependency(long long ParIdx, float DepSta[2])
{
...
void GetDependencyStats(long long ParIdx, LplInt TypIdx1, LplInt TypIdx2, float DepSta[2])
{
	int i, NmbDepBit, TotNmbDep = 0;
	ParSct *par = (ParSct *)ParIdx;
	TypSct *typ1, *typ2;
...
LplInt ParallelMemClear(long long ParIdx, void *PtrArg, size_t siz)
{
...
LplInt LaunchPipeline(long long ParIdx, void *prc, void *PtrArg, LplInt NmbDep, LplInt *DepTab)
{
	int i;
	PipSct *NewPip=NULL;
	ParSct *par = (ParSct *)ParIdx;
...
void WaitPipeline(long long ParIdx)
{
	int PenPip;
	ParSct *par = (ParSct *)ParIdx;
...
void GetLplibInformation(long long ParIdx, LplInt *NmbCpu, LplInt *NmbTyp)
{
	int i;
	ParSct *par = (ParSct *)ParIdx;
...
void ParallelQsort(long long ParIdx, void *base, size_t nel, size_t width, int (*compar)(const void *, const void *))
{
...
{
	unsigned long long IntCrd[3], m=1ULL<<63, cod;
	LplInt i;
...
LplInt HilbertRenumbering(long long ParIdx, LplInt NmbLin, double box[6], double (*crd)[3], unsigned long long (*idx)[2])
{
	LplInt i;
	int j, NewTyp, stat[ (1<<HshBit)+1 ], NmbPip, sum;
	long long bound[ MaxPth ][2], cpt, (*tab)[2];
	size_t NmbByt;
	double len = pow(2,64);
	ParSct *par = (ParSct *)ParIdx;
	ArgSct arg;
	...
	/* Setup the bounding box and a data type, then give a Hilbert code to each entries */
	...
	else
	{
		...
		for(i=0;i<1<<HshBit;i++)
		{
			...
			if(bound[ NmbPip ][0] >= NmbLin / par->NmbCpu)
			{
		...
		for(i=0;i<=NmbPip;i++)
		{
			...
			PipArg[i].nel = cpt;
			PipArg[i].width = 2 * sizeof(long long);
...
{
	unsigned long long IntCrd[2], m=1ULL<<62, cod;
	LplInt i;
...
LplInt HilbertRenumbering2D(long long ParIdx, LplInt NmbLin, double box[4], double (*crd)[2], unsigned long long (*idx)[2])
{
	LplInt i;
	int NewTyp;
	double len = pow(2,62);
...
LplInt ResizeType(long long ParIdx, LplInt TypIdx, LplInt NmbLin)
{
	...
	if(!par || (TypIdx < 1) || (TypIdx > MaxTyp) || !(typ = &par->TypTab[ TypIdx ]) || (NmbLin < typ->NmbLin ) )
		return(0);
...
float LaunchParallel(long long ParIdx, LplInt typ1, LplInt typ2, void *prc, void *PtrArg)
{
	ParSct *par = (ParSct *)ParIdx;
	...
	{
		par->prc = prc;
		CalF77Prc(1, par->TypTab[ typ1 ].NmbLin, 0, par);
	}
	else
	{
		void (*UsrPrc)(LplInt, LplInt, LplInt, void *) = (void (*)(LplInt, LplInt, LplInt, void *))prc;
...
LplInt HilbertRenumbering(long long ParIdx, LplInt NmbLin, double box[6], double (*crd)[3], unsigned long long (*idx)[2])
{
	LplInt i;
	for(i=1;i<=NmbLin;i++)
		idx[i][0] = idx[i][1] = (unsigned long long)i;
	return(1);
}
...
/*----------------------------------------------------------*/
/* Procedures common to serial and parallel modes			*/
/*----------------------------------------------------------*/
...
long long call(initparallel)(LplInt *NmbCpu)
{
	return(InitParallel(*NmbCpu));
}

void call(stopparallel)(long long *ParIdx)
{
	StopParallel(*ParIdx);
}

int call(newtype)(long long *ParIdx, LplInt *NmbLin)
{
	return(NewType(*ParIdx, *NmbLin));
}

void call(freetype)(long long *ParIdx, LplInt *TypIdx)
{
	FreeType(*ParIdx, *TypIdx);
}

int call(begindependency)(long long *ParIdx, LplInt *TypIdx1, LplInt *TypIdx2)
{
	return(BeginDependency(*ParIdx, *TypIdx1, *TypIdx2));
}


void call(adddependency)(long long *ParIdx, LplInt *idx1, LplInt *idx2)
{
	AddDependency(*ParIdx, *idx1, *idx2);
}


void call(enddependency)(long long *ParIdx, float *DepSta)
{
	EndDependency(*ParIdx, DepSta);
}


float call(launchparallel)(long long *ParIdx, LplInt *typ1, LplInt *typ2, void *prc, LplInt *NmbArg, ...)
{
	int i;
	float acc;
	va_list ArgLst;
...
void call(parallelmemclear)(long long *ParIdx, void *ptr, size_t *siz)
{
	ParallelMemClear(*ParIdx, ptr, (size_t)*siz);
}


void call(getlplibinformation)(long long *ParIdx, LplInt *NmbCpu, LplInt *NmbTyp)
{
	GetLplibInformation(*ParIdx, NmbCpu, NmbTyp);
}

int call(launchpipeline)(long long *ParIdx, LplInt *NmbDep, LplInt *DepTab, void *prc, LplInt *NmbArg, ...)
{
	int i, ret;
	va_list ArgLst;
	ParSct *par = (ParSct *)*ParIdx;
...
void call(waitpipeline)(long long *ParIdx)
{
	WaitPipeline(*ParIdx);
}

double call(hilbertrenumbering)(long long *ParIdx, LplInt *NmbLin, double box[6], \
								double (*crd)[3], unsigned long long *Old2New)
{
	int i;
	unsigned long long (*idx)[2];
	...
	if(!(idx = malloc((size_t)(*NmbLin+1) * 2 * sizeof(long long))))
	  return(0.);
...
void call(hilbertrenumbering2d)(long long *ParIdx, LplInt *NmbLin, double box[4], \
								double (*crd)[2], unsigned long long (*idx)[2])
{
