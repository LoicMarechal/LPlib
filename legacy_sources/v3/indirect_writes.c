...
void VerTem(int BegIdx, int EndIdx, int PthIdx, MshSct *msh)
{
	...
	for(i=BegIdx;i<=EndIdx;i++)
		for(j=0;j<4;j++)
			msh->VerTem[ msh->TetVer[i][j] ] += msh->TetTem[i] / msh->VerDeg[ msh->TetVer[i][j] ];
}
...
int main(int ArgCnt, char **ArgVec)
{
	...
	puts("");
	printf("Input mesh : idx = %d, version = %d, dimension = %d\n", InpMsh, ver, dim);
	...
	for(i=1;i<=NmbItr;i++)
	{
		if(!(acc += LaunchParallel(msh.ParIdx, msh.TetTyp, msh.VerTyp, (void *)TetTem, (void *)&msh)))
		{
			puts("Error while launching the parallel loop TetTem.");
			exit(1);
		}

		if(!(acc += LaunchParallel(msh.ParIdx, msh.TetTyp, 0, (void *)VerTem, (void *)&msh)))
		{
