...
void AddVec(int BegIdx, int EndIdx, int PthIdx, AddArgSct *arg)
{
    ...
    // it may help avoiding some false memory conflicts
    vec1 = arg->vec1;
    vec2 = arg->vec2;
    vec3 = arg->vec3;
    ...
    for(i=BegIdx; i<=EndIdx; i++)
        vec3[i] =  exp(log(acos(cos(vec1[i]+1.)))) \
                 + exp(log(acos(cos(vec2[i]+2.))));
}
...
int main(int ArgCnt, char **ArgVec)
{
    ...
    // it stores only pointers to the three vectors
    arg.vec1 = vec1;
    arg.vec2 = vec2;
    arg.vec3 = vec3;

    /* Initialize the LPLib3: if the return value is 0, something went wrong,
    ...
    /* Setup a new data type: provide the instanciation number
       and the size of the vector. The procedure return 0 for a failure
       and a data type index otherwise. This new index must be provided
       to launch any parallel loop on this vector */
    ...
    /* Launch the parallel loop computing v3 = f(v1) + g(v2) 
       ...
       The return value is 0 is something went wrong
       or an acceleration factor otherwise */
